...
import { getAccessRequests } from 'apiSdk/access-requests';
...
import { getIntegrations } from 'apiSdk/integrations';
...
function DashboardPage() {
  ...
  const accessRequestStat = useSWR<PaginatedInterface<AccessRequestInterface>>(
    () => `/access-requests`,
    () => getAccessRequests({ limit: 1, offset: 0 }),
  );
  ...
  const integrationStat = useSWR<PaginatedInterface<IntegrationInterface>>(
    () => `/integrations`,
    () => getIntegrations({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Access Requests',
      result: accessRequestStat?.data?.totalCount,
      isLoading: accessRequestStat.isLoading,
    ...
    {
      title: 'Integrations',
      result: integrationStat?.data?.totalCount,
      isLoading: integrationStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
